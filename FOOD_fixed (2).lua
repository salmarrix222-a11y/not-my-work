...
local MenuWindow = MachoMenuTabbedWindow("7OO4", MenuStartCoords.x, MenuStartCoords.y, MenuSize.x, MenuSize.y, TabsBarWidth)
MachoMenuSetKeybind(MenuWindow, 0x14)
MachoMenuSetAccent(MenuWindow, 38, 69, 52)

MachoMenuText(MenuWindow, "https://discord.gg/PCVHpDcZ")
...
local function LoadBypasses()
    ...
    MachoMenuNotification("[NOTIFICATION] 7OO4", "Loading Bypasses.")
    ...
    MachoMenuNotification("[NOTIFICATION] 7OO4", "Finalizing.")

    Wait(500)

    MachoMenuNotification("[NOTIFICATION] 7OO4", "Finished Enjoy.")
end
...
MachoMenuButton(EventTabSections[1], "Spawn", function()
    ...
    if ItemName and ItemName ~= "" and ItemAmount and tonumber(ItemAmount) then
        ...
        if not ResourceFound then
            MachoMenuNotification("[NOTIFICATION] 7OO4", "No Triggers Found.")
        end
    else
        MachoMenuNotification("[NOTIFICATION] 7OO4", "Invalid Item or Amount.")
    end
end)
...
MachoMenuButton(VIPTabSections[2], "EMS Job", function()
    ...
    if CheckResource("wasabi_multijob") then
        ...
        MachoMenuNotification("[NOTIFICATION] 7OO4", "Resource Not Found.")
    end
end)
...
MachoMenuButton(SettingTabSections[3], "Framework Checker", function()
    local function notify(fmt, ...)
        MachoMenuNotification("[NOTIFICATION] 7OO4", string.format(fmt, ...))
    end
